...
- 보통 책 맨 뒤에 있는 붙어있는 색인표와 같다. 
- 인텍스로 지정된 컬럼의 값이 추가/변경/삭제 될 때 인덱스 정보도 갱신한다.
...
alter table test1
  add constraint unique (no); /* 일단 no를 uk로 지정한다.*/
...
/* auto-inrement 컬럼의 값을 생략하면 마지막 값을 증가시켜서 입력한다.*/
insert into test1(no, name) values(100, 'yyy');

insert into test1(name) values('bbb'); /* no는 101이 된다.*/
