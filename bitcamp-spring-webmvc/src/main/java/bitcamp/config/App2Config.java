...
// => 지정된 패키지를 뒤져서 @Component, @Controller,@Service, @Repository 등 붙은 클래스에 대해 인스턴스를 생성한다.
// IoC컨테이너가 Config를 만들어 디스패처에게 보내줌
...
public class App2Config implements WebMvcConfigurer {
  ...
  // 2) Java Config 설정
  @Bean
  public ViewResolver viewResolver() { // 한 개만 존재해야
  ...
  // @Override
  @Override
  public void configurePathMatch(PathMatchConfigurer configurer) { // 디스패처 서블릿이 호출 MatrixVariable을
                                                                   // 사용하려면 활성화 시켜야함
