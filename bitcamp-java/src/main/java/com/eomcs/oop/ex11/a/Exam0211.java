...
public class Exam0211 {
  ...
  class A {
    ...
    void m1() {
      // 바깥 클래스의 인스턴스 멤버이름이 inner class의 인스턴스 멤버 이름과 같을 경우
      // inner class 안에서는 당연히 inner class 멤버를 가리킨다.
      iValue = 100; // A 객체의 iValue를 의미한다.
      iValue = 100; // A 객체의 iValue를 의미한다.

      // 바깥 클래스의 인스턴스를 가리키고 싶다면, 바깥 클래스의 이름을 명시하라.
      ...
      // => 따라서 inner class의 인스턴스 메소드를 호출하는 시점에는
