...
public class Exam0340 {
  ...
  public static void main(final String[] args) {
    ...
    obj.im(); // im()을 호출할 때 Exma0340 객체를 넘겨준다.
    // 그러면 im() 에서는 this라는 내장 변수에 보관할 것이다.
  }

  int iValue;

  // 인스턴스 메소드의 로컬 클래스 사용법
  void im() {
    // 이 인스턴스 메소드는 Exam0340 객체 주소를 받는 내장 변수 this가 있다.
    
    class A {
      
      // 인스턴스 메소드에서 로컬 클래스를 정의하는 경우
    ...
    final A obj = new A();
    // 컴파일러는 위 코드는 바깥 클래스의 인스턴스 주소를 받는 
    // 생성자 호출 코드로 바뀐다.
