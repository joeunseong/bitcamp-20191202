...
import com.eomcs.oop.ex11.c.Exam0130.JavaFilter;
...
public class Exam0131 {
  ...
  // => 인스턴스 필드를 사용하는 메소드에 대해 인스턴스 메소드로 선언하는 것과 같다.
  //
  ...
  public static void test1() {
    ...
    // 따라서 이 메소드처럼 static 메소드에서 해당 클래스를 사용하려 한다면
    // 오류가 발생할 것이다.
  ...
  public void test2() {
    ...
    // 인스턴스 메소드에서는 non-static nested class의 인스턴스를 생성할 수 있다.
    // => 바깥 클래스의 인스턴스 주소를 앞쪽에 지정한 후 해당 클래스의 인스턴스를 생성하라.
    JavaFilter javaFilter = new JavaFilter(); // 레퍼런스를 선언할 때는 상관없다.
  ...
  public static void main(String[] args) throws Exception {
    ...
    // static 메소드에서는 static이 안붙은 인스턴스 멤버를 사용할 수 없다.
    //javaFilter = new JavaFilter(); // 컴파일 오류!
    
    // static 메소드에서 인스턴스 멤버를 사용하려면
    // 인스턴스를 생성해야한다.
    Exam0131 obj = new Exam0131();
    
    // 그리고 이 인스턴스를 가지고 인스턴스 필드/메소드/중첩클래스를 사용할 수 있다.
