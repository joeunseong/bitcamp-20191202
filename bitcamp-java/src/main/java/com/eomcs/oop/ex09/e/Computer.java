...
public interface Computer {
  ...
  // 
  // default 메소드의 단점:
  // => 인터페이스에 선언하는 메소드는 기본이 추상 메소드이기 때문에
  //    그 인터페이스를 구현하는 클래스는 반드시 해당 메소드를 정의해야 한다.
  //    메소드 정의를 강제하는 것이다.
  // => 그러나 default 메소드는 이미 구현되어 있기 때문에
  //    클래스에서 반드시 정의할 필요는 없다.
  //    즉 강제로 정의하게 만들 수 없는 것이 문제이다.
  // => 그래서 default 메소드의 용도는
  //    기존에 정의된 인터페이스에 새 규칙을 추가할 때
  //    기존 프로젝트에 영향을 끼치지 않기 위해 사용하는 것이다.
