...
# 중간 언어와 가상 머신		
[O]중간 언어와 가상 머신이 등장한 이유와 이점을 설명할 수 있다.	->이식성 높은 컴파일러, 인터프리터 사용 가능성 다른 운영체제에서 사용하는 경우, 하나의                                                                  머신에서 여러명에게 운영체제 환경을 제공
[O]VM과 인터프리터, 런타임, 플레이어, 엔진의 역할을 이해한다.		중간언어나 소스를 실행해주는 프로그램
