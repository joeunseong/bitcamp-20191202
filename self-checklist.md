...
- [ ] CPU 인스트럭션과 실행 파일->(cpu 인스트럭션을 모아놓은 파일), OS의 관계를 이해한다.
- [ ] 같은 CPU에 맞춰 작성된 실행 파일이더라도 OS가 다르면 실행되지 않는 이유를 설명할 수 있다.

#### 중간 언어와 가상 머신

- [ ] 중간 언어와 가상 머신이 등장한 이유와 이점을 설명할 수 있다. -> 기계어만 있으면 운영체제별로 그 기계어를 구성하는 포맷이 다르기 때문에 그에 맞게 기계어를 써야하기 때문에 번거로움이 있어서 / 운영체제 상관없이 쓸 수 있어서 / 이식성 높은 컴파일러, 인터프리터 사용 가능성 다른 운영체제에서 사용하는 경우,  하나의 머신에서 여러명에게 운영체제 환경을 제공
- [ ] VM=인터프리터, 런타임, 플레이어, 엔진의 역할을 이해한다. -> 중간언어나 소스를 실행해주는 프로그램
- [ ] JIT(Just-in-time)과 AOT(ahead-of-time)이 무엇이고 등장한 이유를 이해한다. -> 실행하는 시간에 즉시 중간언어를 기계어로 바꿔 실행함으로써 기계어를 사용하는 것 같은 속도를 냄 /  
...
- [ ] 바이트코드와 클래스 파일(.class)의 관계를 이해한다. `/HelloWorld.class`, `https://medium.com/@davethomas_9528/writing-hello-world-in-java-byte-code-34f75428e0ad` 바이코드가 들어 있는게 클래스 파일
- [ ] 클래스 파일과 JVM의 관계를 이해한다. `$ javap HelloWorld.class`, `$ javap -verbose HelloWorld.class` 클래스 파일을 실행시키는 플레이어
- [ ] JVM을 이용하여 클래스 파일을 실행할 수 있다. $ java HelloWorld -> JVM이 바이트 코드를 읽어서 클래스 파일을 실행함

#### 프로그래밍 언어와 컴파일

- [ ] 컴파일 방식 프로그래밍이 등장한 이유를 이해한다. ->인간이 이해하기 쉬운 언어로 바꾸고 실행시키기 위해서
...
- [ ] src 디렉토리 외에 다른 디렉토리나 파일의 용도를 이해한다.  *gradle build
...
- [x ] 문자 '_' 를 사용하여 정수 리터럴의 자릿수를 표현할 수 있고, _ 문자의 적절한 위치를 안다. `ex03/Exam22.java`
- [ ] 메모리에 값을 저장하기 위해 데이터를 2진수로 변환해야 하는 이유를 설명할 수 있다.
- [x ] 메모리 크기에 따라 정수를 저장할 수 있는 범위가 다른 이유를 이해한다.
- [ x] 메모리 크기에 따라 정수 리터럴을 4바이트와 8바이트로 구분하여 표현할 수 있다. `ex03/Exam23.java`
- [ x] 정수 리터럴이 메모리 크기를 벗어날 때 어떤 문제가 발생하는지 안다. `ex03/Exam23.java`
- [x ] 정수 리터럴이 메모리에 어떻게 저장되는지 이해한다. `ex03/Exam24.java`
- [ ] 정수를 2진수로 표현하는 다양한 방법을 이해한다. (Sign-magnitude, 1의 보수, 2의 보수, Excess-K 방식)
- [ ] 부동소수점 리터럴을 다양한 방식으로 표기할 수 있다. `ex03/Exam31.java`
- [ x] 메모리 크기에 따라 부동소수점 리터럴을 4바이트와 8바이트로 구분하여 표현할 수 있다.`ex03/Exam32.java
- [ x] 메모리 크기에 따라 부동소수점의 최소/최대 크기를 알아낼 수 있다. `ex03/Exam33.java`
- [ x] 부동소수점이 메모리에 저장되는 원리를 이해한다. (IEEE 754 규칙에 따라 부동소수점이 2진수로 변환되는 것을 이해한다.) `ex03/Exam34.java`
- [x ] 부동소수점이 때론 완전하게 2진수로 변환되지 못하는 이유를 안다.
- [x ] 자바에서 부동소수점을 IEEE 754 규칙에 따라 2진수로 변환된 것을 확인해 볼 수 있다. `ex03/Exam35.java`
