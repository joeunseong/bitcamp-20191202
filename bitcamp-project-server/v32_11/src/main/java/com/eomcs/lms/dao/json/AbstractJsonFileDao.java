...
// 서브 클래스에게 공통 필드나 메소드를 상속해주는 역할을 하기 때문에
// => abstract
// 서브 클래스에게 상속해주는 메소드 중에서 indexOf()처럼 구현되지 않은 메소드가 있기 때문에
// => abstract
//
public abstract class AbstractJsonFileDao<T> {
  ...
  protected void loadData() {
    // 같은 패키지에 있는 클래스 사용 가능, 패키지 달라도 같은 서브 클래스면 사용 가능
    ...
    try (BufferedReader in = new BufferedReader(new FileReader(file))) {
      ...
      // => 수퍼 클래스에 제네릭이 적용된 경우 실제 타입은 다음과 같다.
  ...
  protected void saveData() {
    ...
    try (BufferedWriter out = new BufferedWriter(new FileWriter(file))) {
      out.write(new Gson().toJson(list)); // toJosn으로 string으로 만들어서 리턴
  ...
  // 서브 클래스들의 공통 메소드이기 때문에
  // => 수퍼 클래스에 정의한다.
  // 서브 클래스에서 접근할 수 있어야 하기 때문에
  // => protected
  // 서브 클래스마다 구현 방법이 다르기 때문에
  // => abstract
  // =>(수퍼 클래스에서 구현할 수도 없고, 구현해봐야 소용없다.)
  // 서브 클래스에서 반드시 구현해야 할 메소드이기 때문에
  // => abstract
  //
