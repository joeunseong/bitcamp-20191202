...
public class LessonServiceImpl2 implements LessonService {
  ...
  public LessonServiceImpl2(LessonDao lessonDao) {
    this.lessonDao = lessonDao;
  }

  @Override
  public Lesson get(int no) throws Exception {
    // 이 메소드는 사실 별도의 작업을 하지 않는다.
    // 그냥 DAO의 메소드를 호출하여 리턴 받은 값을 그대로 다시 리턴할 뿐이다.
    // 이렇게 아무런 일도 안하는 메소드를 만든 이유가 무엇인가?
    ...
    //    일관성이 없어 유지보수가 어려워진다.
    // => 이처럼 때로는 아무일도 하지 않는 서비스 클래스를 정의하더라도 
