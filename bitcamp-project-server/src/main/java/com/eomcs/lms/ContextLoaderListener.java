...
// 이 클래스의 객체에 대해 메소드를 호출한다.
// 즉 이 클래스는 서블릿 컨테이너의 시작과 종료에 대해 알림을 받는다.
// 조건:
// => javax.servlet.ServletContextListener 인터페이스를 구현해야한다.
//
...
public class ContextLoaderListener implements ServletContextListener {
  ...
  public void contextInitialized(ServletContextEvent sce) {
    // 서블릿 컨테이너가 실행할 때
    // 실행한 사실을 알리기 위하여 모든 SerlvetContextListener에 대해
    // 이 메소드를 호출한다.
    // => 보통 이 메소드에서는 웹 애플리케이션이 실행되는 동안 사용할 객체를 준비한다.
    ...
    try {
      ...
      for (String beanName : beanNames) {
        ...
        if (method != null) {
          ...
          // @RequestMapping 메소드가 들어있는 클래스 이름을 로그로 남긴다.
