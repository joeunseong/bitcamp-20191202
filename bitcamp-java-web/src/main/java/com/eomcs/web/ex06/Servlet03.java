...
public class Servlet03 extends HttpServlet {
  ...
  // // 서블릿 객체가 생성될 때 뭔가 준비하는 작업을 해아한다면,
  // // 보통 이 메소드를 오버라이딩 할 것이다.
  // //
  // // 문제는 이 메소드가 호출될 때 넘어오는 값(config)을 인스턴스 필드에 보관했다가,
  // // 나중에 getServletConfig()가 호출될 때 리턴하도록 코드를 작성해야한다.
  // // 이런 작업이 번거롭다.
  // //
  // // 이런 불편함을 없애기 위해서 GenericServlet은
  // // 미리 이 메소드에 해당 코드를 작성해두었다.
  // // 그리고 추가적으로 파라미터 값을 받지 않는 init()를 호출하도록 구현하였다.
  // //
  // // 결론?
  // // => 그러니 개발자는 서블릿 객체가 생성될 때 뭔가 작업을 수행하고 싶다면,
  // // 이 메소드를 직접 오버라이딩 하지 말고,
  // // 이 메소드가 호출하는 다른 init()를 오버라이딩 하라!
  // //
  // super.init(config);
  // }
  ...
  public void init() throws ServletException {
    ...
    // 이 소스를 변경하고 다시 컴파일 해야하는 번거로움이 있다.
