...
public class Stack<E> implements Cloneable {// 복제(clone)하는게 가능하다고 표기한 것
  ...
  public E pop() {
    ...
    this.elementData[size] = null; // 안쓰는걸 가비지로 만들기 위해서 => 메모리 관리
    return value; 
  }
  ...
  /*
  @Override
  public Stack clone() { // 얕은 복제를 하게 되면 원본 배열을 복제하지 않고 가리키기만 하기 때문에
                        //원본 값을 건들이게 된다. 그래서 때문에 깊은 복제가 필요
    try {
