...
public class LessonHandler {
  ...
  public void updateLesson() {
    ...
    newLesson.setTitle(
        prompt.inputString(String.format("수업명(%s)? ", oldLesson.getTitle()), 
            oldLesson.getTitle()));

    newLesson.setDescription(prompt.inputString("설명?", oldLesson.getDescription()));

    newLesson.setStartDate(
        prompt.inputDate(String.format("시작일(%s)? ", oldLesson.getStartDate()), 
            oldLesson.getStartDate()));

    newLesson.setEndDate(
        (prompt.inputDate(String.format("종료일(%s)? ", oldLesson.getEndDate()), 
    ...
    newLesson.setDayHours(
        (prompt.inputInt(String.format("일수업시간(%s)? ", oldLesson.getDayHours()), 
